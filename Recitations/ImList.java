...
public class ImList<E> implements Iterable<E> {
    ...
    public ImList<E> limit(int n) {
        ImList<E> newList = new ImList<E>();
        int size = Math.min(Math.max(n, 0), this.size());
    ...
    public <R> ImList<R> map(Function<? super E, ? extends R> mapper) {
        ImList<R> newList = new ImList<R>();

        for (E e : this) {
            newList.elems.add(mapper.apply(e));
        }
        return newList;
    }

    public <R> ImList<R> flatMap(Function<? super E, ? extends ImList<? extends R>> mapper) {
        ImList<R> newList = new ImList<R>();

        for (E e : this) {
            newList = newList.addAll(mapper.apply(e));
        }
        return newList;
    }
    ...
    public void forEach(Consumer<? super E> consumer) {
        ...
        // or this.elems.forEach(consumer);
    }

    public <R> R reduce(R seed, BiFunction<? super R, ? super E, ? extends R> bif) {
        for (E e: this) {
            seed = bif.apply(seed, e);
        }
        return seed;
    }
