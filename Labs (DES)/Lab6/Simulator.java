...
class Simulator {
    ...
    Simulator(double numOfServers, int qmax, 
    ...
    ImList<Server> generateServers() {
        ...
        for (int i = 1; i <= this.numOfServers; i++) {
            allServers = allServers.add(
                new Server(String.valueOf(i), 0.0, this.qmax, this.restTimes));
        }
        return allServers;
    }
    ...
    public String simulate() {
        ...
        while (!eventsPQ.isEmpty()) {
            ...
            if ((event.isServe() || event.isDone()) && !eventsPQ.isEmpty()) {
                ...
                while (nextEvent.isServe() && nextEvent.isSameTime(event) 
                    && nextEvent.isSameServer(event)) {
                    //System.out.println(eventsPQ);
                    eventsPQ = eventsPQ.poll().second();
                    Pair<Boolean, ? extends Event> pair = 
                        nextEvent.updateServeEventTime(nextAvailableTime);
                    Event addEvent = pair.second();
